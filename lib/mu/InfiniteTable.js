 ...
 * Infinite MUI table
 */
export const InfiniteTable = React.forwardRef(({ innerClassName, footerRenderer, headerRenderer, height, hideHeader, itemRenderer, onItemClick, orderIndex, padding, rowHeight, selectable, sortable, ...rest }, ref) => {
    ...
    const cachedOrderIndex = Utils.cacheSessionDataParse(Utils.getLocationKey(cacheOrderIndexKey));
    // Local order index
    ...
    // Field sort
    const createSortHandler = (field, type, index) => (event) => {
        var _a;
    ...
    const tableItemRenderer = (p) => {
        if (p.data) {
                ...
                if (hasHeader && p.index === 0) {
                    ...
                    if (headerRenderer) {
                        ...
                        rows = React.createElement(React.Fragment, null,
                            ...
                            p.layouts.map((c, columnIndex) => (React.createElement(TableCell, { component: "div", key: 'head' + c.field, className: classes.tableCell, align: searchLayoutAlign(c.align) }, sortable && c.sort != null ? (React.createElement(TableSortLabel, Object.assign({}, createSort(c.sort), { className: classes.tableRowClick, onClick: createSortHandler(c.field, c.type, c.sort) }), c.label || c.field)) : (React.createElement(React.Fragment, null, c.label || c.field))))));
                    }
                    else {
                        rows = React.createElement(TableCell, { component: "div", className: classes.tableCell },
                            React.createElement(CircularProgress, { size: 20 }));
            ...
            return (React.createElement("div", { style: p.style, className: Utils.mergeClasses(...classNames), "data-index": p.index, onClick: itemClickHandler }, rows));
        }
        else {
            return React.createElement(React.Fragment, null);
        }
