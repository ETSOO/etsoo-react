...
export function StepperForm(props) {
    // Destruct
    const { buttons, maxWidth, padding, promptForExit, steps, submitHandler } = props;
    // Styles
                ...
                default:
                    if (currentMethods) {
                        currentMethods.collectData().then(data => {
                            ...
                            if (updateStep(data)) {
                                // Check completion
                                const activeIndex = steps.findIndex((_step, index) => !completedSteps.has(index));
                                // Active index found
                                if (activeIndex !== -1) {
    ...
    return (React.createElement("div", { className: classes.root },
        ...
        React.createElement(Container, { component: "main", className: classes.paper, maxWidth: maxWidth == null ? false : maxWidth },
            steps.map((step, index) => {
                ...
                React.createElement("div", { key: index, hidden: !active, className: classes.formContainer }, React.createElement(step.form, { formData, formReady: (active ? formReady : undefined) })));
            }),
            buttonElements)));
}
