...
export class LoginController extends EntityController {
     ...
     */
    async login(model, dataCallback = null) {
        ...
        const result = this.loginFormat ? this.loginFormat(data) : this.formatResult(data);
     ...
     */
    async loginToken(model, dataCallback = null) {
        const { method, currentLanguage: languageCid } = this.singleton.settings;
        const post = Object.assign(model, { method, languageCid });
        const data = (await this.api.post('LoginToken', post)).data;
        const result = this.loginTokenFormat ? this.loginTokenFormat(data) : this.formatResult(data);
    ...
    async refreshToken() {
        var _a;
        const result = this.formatResult((await this.api.put('RefreshToken')).data);
        if (result.ok) {
            // Update token
