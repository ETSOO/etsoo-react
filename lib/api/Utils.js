 ...
 * @param word Word
 */
const formatUpperLetter = (word) => word.charAt(0).toUpperCase() + word.slice(1);
/**
 * Form data to object
 ...
 * @param key Key
 */
const getLocationKey = (key) => `${window.location.href}:${key}`;
/**
 * Join items as a string
 * @param items Items
 */
const joinItems = (...items) => items.filter(item => item != null).join(', ');
/**
 * Merge class names
 * @param classNames Class names
 */
const mergeClasses = (...classNames) => classNames.filter(name => name != null).join(' ');
/**
 * Parse float value
