...
export function InfiniteGrid(props) {
    ...
    const isItemLoaded = (index) => state.loaded || index < state.items.length;
    // Load more items
    ...
    return (React.createElement(InfiniteLoader, { isItemLoaded: isItemLoaded, itemCount: itemCount, loadMoreItems: loadMoreItems, minimumBatchSize: records, threshold: records + 5 }, ({ onItemsRendered, ref }) => (React.createElement(FixedSizeGrid, { className: props.className, columnCount: columnCount, columnWidth: localColumnWidth, direction: direction, height: props.height, initialScrollLeft: props.initialScrollLeft, initialScrollTop: props.initialScrollTop, itemKey: itemKey, onScroll: props.onScroll, onItemsRendered: ({ visibleRowStartIndex, visibleRowStopIndex, overscanRowStopIndex, overscanRowStartIndex }) => {
            onItemsRendered({
        ...
        }, rowCount: rowCount, ref: ref, rowHeight: props.rowHeight, width: localWidth }, itemRenderer))));
}
