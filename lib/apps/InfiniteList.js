...
// Format items
const formatItems = (items, hasHeader, hasFooter) => {
    var _a;
    // Header
    if (hasHeader) {
        // Insert header item
...
export const InfiniteList = React.forwardRef((props, ref) => {
    ...
    // Item key
    const itemKey = (index, data) => InfiniteListKey(index, data, props.itemKey || 'id');
    // Loader reference
    ...
    let defaultState = props.tryCache
        ? Utils.cacheSessionDataParse(Utils.getLocationKey(uniqueKey))
        : undefined;
    if (!defaultState) {
    ...
    const innerElementType = (p) => {
        const { style, ...rest } = p;
        const { innerClassName, padding } = props;
        if (padding) {
            style.height = Utils.parseNumber(style.height) + 2 * padding;
        }
        return (React.createElement("div", Object.assign({ ref: domRef, className: innerClassName, style: style }, rest)));
    };
